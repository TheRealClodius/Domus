...
describe('useDragEntity', () => {
	...
	it('cleans body styles and pending RAF on unmount mid-drag', () => {
		useEntityStore.getState().upsert(makeEntity({ id: 'e1' }))
		const rafSpy = vi.spyOn(global, 'requestAnimationFrame').mockReturnValue(42)
		const cancelSpy = vi.spyOn(global, 'cancelAnimationFrame').mockImplementation(() => {})

		const wrapper = document.createElement('div')
		wrapper.setAttribute('data-entity-wrapper', '')
