...
describe('POST /api/entities/[id]/call', () => {
	...
	describe('chat tool authorization & SSRF', () => {
		...
		it('send_image rejects non-image content-type responses', async () => {
			const fetchSpy = vi.spyOn(global, 'fetch').mockResolvedValueOnce(
			...
			expect(res.status).toBe(400)
			expect(json.ok).toBe(false)
			expect(json.error).toBe('invalid_image_content_type')
			fetchSpy.mockRestore()
		})

		it('send_image rejects oversized responses via content-length', async () => {
			const fetchSpy = vi.spyOn(global, 'fetch').mockResolvedValueOnce(
