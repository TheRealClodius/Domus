...
const MAX_FETCHED_IMAGE_BYTES = 10 * 1024 * 1024 // 10 MB
...
async function readResponseBodyWithLimit(response: Response, maxBytes: number): Promise<Buffer | null> {
		...
		if (entity.type === 'chat') {
			...
			if (toolName === 'send_image') {
				...
				try {
					...
					const boundedBuffer = await readResponseBodyWithLimit(resp, MAX_FETCHED_IMAGE_BYTES)
					if (!boundedBuffer) {
						return NextResponse.json({ ok: false, error: 'image_too_large' }, { status: 400 })
					}
					imageBuffer = boundedBuffer
				} catch {
					return NextResponse.json({ ok: false, error: 'image_fetch_failed' }, { status: 400 })
				}
