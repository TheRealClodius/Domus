...
function CloseButton({ onClick }: { onClick: () => void }) {
	return (
		<button
			...
			onClick={onClick}
			aria-label="Close"
			className="absolute top-4 left-4 size-8 rounded-full flex items-center justify-center text-on-surface-muted hover:bg-surface transition-colors"
		>
			<X className="size-4" />
		</button>
	)
...
function JoinContent({ onClose }: { onClose: () => void }) {
	return (
		<div className="h-full bg-surface flex flex-col items-center justify-center p-6 relative">
			...
			<Input placeholder="Invite code" className="max-w-xs w-full bg-surface mb-4" />
...
function CreateContent({ onClose }: { onClose: () => void }) {
	return (
		<div className="h-full bg-surface flex flex-col items-center justify-center p-6 relative">
			<CloseButton onClick={onClose} />
			<div className="size-20 rounded-full bg-surface flex items-center justify-center mb-5">
				<Camera className="size-6 text-on-surface-muted" />
			</div>
			<h2 className="text-heading text-on-surface mb-4">Create a new group</h2>
			<Input placeholder="Group name" className="max-w-xs w-full bg-surface mb-6" />
			<div className="w-full max-w-xs border-t border-outline-variant pt-4 mb-4">
				...
				<div className="bg-surface rounded-md px-3 py-2 mb-3 font-mono text-body-sm text-on-surface-muted select-all">
					— invite code will appear here —
				</div>
				<Input placeholder="Enter email addresses..." className="w-full bg-surface" />
			</div>
			<div className="flex items-center gap-2">
				<Button variant="pill-active" size="pill" disabled>
					Create
